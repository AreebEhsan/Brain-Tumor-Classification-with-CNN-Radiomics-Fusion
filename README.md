...
This repository contains an end-to-end research pipeline for MRI brain tumor classification that combines a pretrained CNN (MobileNetV2) with engineered radiomics features and demonstrates interpretability via Grad-CAM. The project includes data preparation, model training, evaluation (accuracy & macro-F1), robustness experiments (noise / resolution), and visualization of Grad-CAM overlays.
...
You can adapt the notebook logic into scripts using the same function calls the notebook defines.
...
Consider exporting the numeric results as CSVs in `results/` for reproducibility and plotting outside the notebook.
...
Tips:
- Aggregate Grad-CAMs for a class by averaging across samples to identify consistent salient regions.
- Save overlays as PNGs in `results/` for reporting.

---

## File / Cell Map (where to find things)

- `cnn_baseline_gradcam.ipynb` — main notebook containing the full pipeline (imports, dataset download, transforms, model, training, radiomics, fusion, Grad-CAM and experiments).
- `brain_tumor_mini.ipynb` — alternate or simplified notebook for quick experiments and visualization.
- `models/` — saved checkpoints (suggested).
- `results/` — saved plots, Grad-CAM images, CSV metrics (suggested).

---

## Troubleshooting

- If notebook fails to import a library: install it in the activated environment (see `requirements.txt`).
- Dataset path errors: confirm the `path` variable points to the dataset root which must contain `Training/` and `Testing/` subfolders.
- GPU not used: ensure `torch.cuda.is_available()` is True and move model and tensors to `device = torch.device("cuda")`.
...
Maintainer: Areeb Ehsan

If you want me to generate a `requirements.txt`, `LICENSE`, or insert this README as a markdown cell at the top of the main notebook, tell me which and I'll add it.
